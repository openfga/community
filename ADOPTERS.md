...
| [Authorizer](https://authorizer.dev) | [Authorizer](https://github.com/authorizerdev/authorizer) is an open-source, self-hosted authentication and authorization server. It embeds OpenFGA in-process as its fine-grained authorization engine, so every deployment ships ReBAC out of the box: authorization models and relationship tuples are managed through Authorizer's GraphQL and gRPC APIs and its admin dashboard, and permission checks are available to applications alongside login, sessions, and token issuance. |
